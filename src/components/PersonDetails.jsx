...
const PersonDetails = () => {
  ...
  const { id } = useParams(); // Get person/tv ID from URL
  const dispatch = useDispatch();
  const { info } = useSelector((state) => state.person); // Extract person info from Redux state

  useEffect(() => {
    dispatch(asyncLoadperson(id)); // Fetch person data when the component loads
    return () => dispatch(removeperson()); // Clear person data when the component unmounts
  }, [id]);
    ...
    >
      ...
      <div className="w-full flex mt-10 gap-10">
        ...
        <div className="content text-white flex flex-col justify-between">
          ...
          <h2 className="text-2xl mt-6 mb-6">person Translations</h2>
