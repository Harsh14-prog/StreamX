...
const TopNav = () => {
  ...
  return (
    <div className='z-100 w-[80%] h-[10vh] relative flex items-center mx-auto'>
      ...
      {query.length > 0 && (
        <i onClick={() => setquery("")} className="ri-close-line text-3xl text-zinc-400 hover:text-white cursor-pointer"></i>
      )}
      ...
      {query.length > 0 && search?.length > 0 && (
        <div className='w-[50%] max-h-[50vh] bg-[#1F1E24] absolute top-[100%] left-[5%] overflow-y-auto p-3 rounded'>
          {search.map((item, index) => (
            <Link
              key={index}
              to={`/${item.media_type}/details/${item.id}`} // ✅ Corrected path
              className='w-full p-4 flex justify-start font-semibold text-zinc-600 hover:text-white duration-300 hover:bg-[#3a3a3a49] transition-all items-center border-b-2 border-zinc-600'
            >
              <img
